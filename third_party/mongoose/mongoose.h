...
#define MG_ENABLE_TCPIP 1 // Enable built-in TCP/IP stack
    ...
    !defined(MG_ENABLE_DRIVER_STM32N)
...
#if (MG_ARCH == MG_ARCH_CMSIS_RTOS1 || MG_ARCH == MG_ARCH_CMSIS_RTOS2) && !defined MG_ENABLE_RL &&                     \
    (!defined(MG_ENABLE_LWIP) || !MG_ENABLE_LWIP) && (!defined(MG_ENABLE_TCPIP) || !MG_ENABLE_TCPIP)
...
#if (!defined(MG_ENABLE_TCPIP) || !MG_ENABLE_TCPIP) && (!defined(MG_ENABLE_LWIP) || !MG_ENABLE_LWIP) &&                \
...
#define MG_SOCK_PENDING(errcode)                                                                                       \
	(((errcode) < 0) &&                                                                                                \
	 (WSAGetLastError() == WSAEINTR || WSAGetLastError() == WSAEINPROGRESS || WSAGetLastError() == WSAEWOULDBLOCK))

#define MG_SOCK_RESET(errcode) (((errcode) < 0) && (WSAGetLastError() == WSAECONNRESET))
...
#define MG_SOCK_PENDING(errcode)                                                                                       \
	((errcode) == -pdFREERTOS_ERRNO_EWOULDBLOCK || (errcode) == -pdFREERTOS_ERRNO_EISCONN ||                           \
	 (errcode) == -pdFREERTOS_ERRNO_EINPROGRESS || (errcode) == -pdFREERTOS_ERRNO_EAGAIN)
...
#define MG_SOCK_RESET(errcode) ((errcode) == BSD_ECONNABORTED || (errcode) == BSD_ECONNRESET)
...
#define MG_U32(a, b, c, d)                                                                                             \
	(((uint32_t)((a) & 255) << 24) | ((uint32_t)((b) & 255) << 16) | ((uint32_t)((c) & 255) << 8) |                    \
	 (uint32_t)((d) & 255))

#define MG_IPV4(a, b, c, d) mg_htonl(MG_U32(a, b, c, d))

#define MG_IPV6(a, b, c, d, e, f, g, h)                                                                                \
	{(uint8_t)((a) >> 8), (uint8_t)(a), (uint8_t)((b) >> 8), (uint8_t)(b), (uint8_t)((c) >> 8), (uint8_t)(c),          \
	 (uint8_t)((d) >> 8), (uint8_t)(d), (uint8_t)((e) >> 8), (uint8_t)(e), (uint8_t)((f) >> 8), (uint8_t)(f),          \
...
#define MG_LOAD_BE16(p) ((uint16_t)(((uint16_t)MG_U8P(p)[0] << 8U) | MG_U8P(p)[1]))
#define MG_LOAD_BE24(p) ((uint32_t)(((uint32_t)MG_U8P(p)[0] << 16U) | ((uint32_t)MG_U8P(p)[1] << 8U) | MG_U8P(p)[2]))
#define MG_LOAD_BE32(p)                                                                                                \
	((uint32_t)(((uint32_t)MG_U8P(p)[0] << 24U) | ((uint32_t)MG_U8P(p)[1] << 16U) | ((uint32_t)MG_U8P(p)[2] << 8U) |   \
	            MG_U8P(p)[3]))
#define MG_LOAD_BE64(p)                                                                                                \
	((uint64_t)(((uint64_t)MG_U8P(p)[0] << 56U) | ((uint64_t)MG_U8P(p)[1] << 48U) | ((uint64_t)MG_U8P(p)[2] << 40U) |  \
	            ((uint64_t)MG_U8P(p)[3] << 32U) | ((uint64_t)MG_U8P(p)[4] << 24U) | ((uint64_t)MG_U8P(p)[5] << 16U) |  \
...
struct mg_connection* mg_connect_svc(struct mg_mgr* mgr, const char* url, mg_event_handler_t fn, void* fn_data,
...
long mg_http_upload(struct mg_connection* c, struct mg_http_message* hm, struct mg_fs* fs, const char* dir,
                    size_t max_size);
...
int mg_tls_x25519(uint8_t out[X25519_BYTES], const uint8_t scalar[X25519_BYTES], const uint8_t x1[X25519_BYTES],
                  int clamp);
...
//
//
int mg_aes_gcm_encrypt(unsigned char* output, const unsigned char* input, size_t input_length, const unsigned char* key,
                       const size_t key_len, const unsigned char* iv, const size_t iv_len, unsigned char* aead,
                       size_t aead_len, unsigned char* tag, const size_t tag_len);

int mg_aes_gcm_decrypt(unsigned char* output, const unsigned char* input, size_t input_length, const unsigned char* key,
                       const size_t key_len, const unsigned char* iv, const size_t iv_len, unsigned char* aead,
                       size_t aead_len, const unsigned char* tag, const size_t tag_len);
...
*/
int mg_uecc_sign(const uint8_t* private_key, const uint8_t* message_hash, unsigned hash_size, uint8_t* signature,
                 MG_UECC_Curve curve);
...
typedef struct MG_UECC_HashContext {
	...
	void (*update_hash)(const struct MG_UECC_HashContext* context, const uint8_t* message, unsigned message_size);
	void (*finish_hash)(const struct MG_UECC_HashContext* context, uint8_t* hash_result);
...
Returns 1 if the signature generated successfully, 0 if an error occurred.
*/
int mg_uecc_sign_deterministic(const uint8_t* private_key, const uint8_t* message_hash, unsigned hash_size,
                               const MG_UECC_HashContext* hash_context, uint8_t* signature, MG_UECC_Curve curve);
...
*/
int mg_uecc_verify(const uint8_t* public_key, const uint8_t* message_hash, unsigned hash_size, const uint8_t* signature,
                   MG_UECC_Curve curve);
...
#if ((MG_UECC_PLATFORM == mg_uecc_arm || MG_UECC_PLATFORM == mg_uecc_arm_thumb ||                                      \
...
mg_uecc_word_t mg_uecc_vli_isZero(const mg_uecc_word_t* vli, wordcount_t num_words);

/* Returns nonzero if bit 'bit' of vli is set. */
mg_uecc_word_t mg_uecc_vli_testBit(const mg_uecc_word_t* vli, bitcount_t bit);

/* Counts the number of bits required to represent vli. */
bitcount_t mg_uecc_vli_numBits(const mg_uecc_word_t* vli, const wordcount_t max_words);
...
mg_uecc_word_t mg_uecc_vli_equal(const mg_uecc_word_t* left, const mg_uecc_word_t* right, wordcount_t num_words);

/* Constant-time comparison function - secure way to compare long integers */
/* Returns sign of left - right, in constant time. */
cmpresult_t mg_uecc_vli_cmp(const mg_uecc_word_t* left, const mg_uecc_word_t* right, wordcount_t num_words);

/* Computes vli = vli >> 1. */
void mg_uecc_vli_rshift1(mg_uecc_word_t* vli, wordcount_t num_words);

/* Computes result = left + right, returning carry. Can modify in place. */
mg_uecc_word_t mg_uecc_vli_add(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                               wordcount_t num_words);

/* Computes result = left - right, returning borrow. Can modify in place. */
mg_uecc_word_t mg_uecc_vli_sub(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                               wordcount_t num_words);

/* Computes result = left * right. Result must be 2 * num_words long. */
void mg_uecc_vli_mult(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                      wordcount_t num_words);
...
/* Computes result = (left + right) % mod.
   ...
   mod. */
void mg_uecc_vli_modAdd(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                        const mg_uecc_word_t* mod, wordcount_t num_words);

/* Computes result = (left - right) % mod.
   Assumes that left < mod and right < mod, and that result does not overlap
   mod. */
void mg_uecc_vli_modSub(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                        const mg_uecc_word_t* mod, wordcount_t num_words);

/* Computes result = product % mod, where product is 2N words long.
   Currently only designed to work for mod == curve->p or curve_n. */
void mg_uecc_vli_mmod(mg_uecc_word_t* result, mg_uecc_word_t* product, const mg_uecc_word_t* mod,
                      wordcount_t num_words);
...
/* Computes result = (left * right) % mod.
   Currently only designed to work for mod == curve->p or curve_n. */
void mg_uecc_vli_modMult(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                         const mg_uecc_word_t* mod, wordcount_t num_words);

/* Computes result = (left * right) % curve->p. */
void mg_uecc_vli_modMult_fast(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* right,
                              MG_UECC_Curve curve);

/* Computes result = left^2 % mod.
   Currently only designed to work for mod == curve->p or curve_n. */
void mg_uecc_vli_modSquare(mg_uecc_word_t* result, const mg_uecc_word_t* left, const mg_uecc_word_t* mod,
                           wordcount_t num_words);

/* Computes result = left^2 % curve->p. */
void mg_uecc_vli_modSquare_fast(mg_uecc_word_t* result, const mg_uecc_word_t* left, MG_UECC_Curve curve);

/* Computes result = (1 / input) % mod.*/
void mg_uecc_vli_modInv(mg_uecc_word_t* result, const mg_uecc_word_t* input, const mg_uecc_word_t* mod,
                        wordcount_t num_words);
...
/* Multiplies a point by a scalar. Points are represented by the X coordinate
   ...
   curve->num_words). */
void mg_uecc_point_mult(mg_uecc_word_t* result, const mg_uecc_word_t* point, const mg_uecc_word_t* scalar,
                        MG_UECC_Curve curve);

/* Generates a random integer in the range 0 < random < top.
   Both random and top have num_words words. */
int mg_uecc_generate_random_int(mg_uecc_word_t* random, const mg_uecc_word_t* top, wordcount_t num_words);
...
*/
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_encrypt(uint8_t* restrict cipher_text,
                                                       const uint8_t key[RFC_8439_KEY_SIZE],
                                                       const uint8_t nonce[RFC_8439_NONCE_SIZE],
                                                       const uint8_t* restrict ad, size_t ad_size,
                                                       const uint8_t* restrict plain_text, size_t plain_text_size);
...
*/
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_decrypt(uint8_t* restrict plain_text,
                                                       const uint8_t key[RFC_8439_KEY_SIZE],
                                                       const uint8_t nonce[RFC_8439_NONCE_SIZE],
                                                       const uint8_t* restrict ad, size_t ad_size,
...
int mg_rsa_mod_pow(const uint8_t* mod, size_t modsz, const uint8_t* exp, size_t expsz, const uint8_t* msg, size_t msgsz,
                   uint8_t* out, size_t outsz);
int mg_rsa_crt_sign(const uint8_t* em, size_t em_len, const uint8_t* dP, size_t dP_len, const uint8_t* dQ,
                    size_t dQ_len, const uint8_t* p, size_t p_len, const uint8_t* q, size_t q_len, const uint8_t* qInv,
                    size_t qInv_len, uint8_t* signature, size_t sig_len);
#endif // TLS_RSA_H
...
enum mg_l2addrtype { MG_TCPIP_L2ADDR_BCAST, MG_TCPIP_L2ADDR_MCAST, MG_TCPIP_L2ADDR_MCAST6 };
...
#if MG_ENABLE_TCPIP && (defined(MG_ENABLE_DRIVER_IMXRT10) && MG_ENABLE_DRIVER_IMXRT10) ||                              \
...
#if MG_ENABLE_TCPIP && defined(MG_ENABLE_DRIVER_NXP_WIFI) && MG_ENABLE_DRIVER_NXP_WIFI
...
#if MG_ENABLE_TCPIP && (defined(MG_ENABLE_DRIVER_RA6) && MG_ENABLE_DRIVER_RA6) ||                                      \
...
#if MG_ENABLE_TCPIP && (defined(MG_ENABLE_DRIVER_CYW_SDIO) && MG_ENABLE_DRIVER_CYW_SDIO)
...
struct mg_tcpip_sdio {
	...
	bool (*xfr)(void*, bool write, uint32_t arg, uint16_t blksz, uint32_t*, uint32_t len, uint32_t* r);
};
...
bool mg_sdio_set_blksz(struct mg_tcpip_sdio* sdio, unsigned int f, uint16_t blksz);
...
bool mg_sdio_transfer(struct mg_tcpip_sdio* sdio, bool write, unsigned int f, uint32_t addr, void* data, uint32_t len);

#endif

#if MG_ENABLE_TCPIP && defined(MG_ENABLE_DRIVER_ST67W6) && MG_ENABLE_DRIVER_ST67W6
